...
export default function PostCard({ post }) {
  ...
  return (
    <div className='p-1 bg-gray-200 shadow-sm flex flex-col'>
      <img className='object-contain' src={thumbnail || './default.jpg'} alt={title} />
      <div className="p-2">
        <h1 className='text-lg font-semibold text-black-700'>{title}</h1>
        <p className='text-gray-500'>{meta.substring(0,75)+"..."}</p>
        ...
        <div className="flex space-x-3">
          ...
          <button className='w-8 h-8 rounded-full bg-red-400 hover:bg-blue-600 flex justify-center items-center text-white'>
            <BsTrash/>
          </button>
        </div>
      </div>
